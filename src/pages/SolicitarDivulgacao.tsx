...
const SolicitarDivulgacao = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar data/hora de publicação
    const publicacaoDate = new Date(dataPublicacao);
    const now = new Date();
    
    // Verificar se é dia útil (simplificado para exemplo - apenas verifica finais de semana)
    const dayOfWeek = publicacaoDate.getDay();
    const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
    
    // Verificar horário comercial (8h às 12h e 14h às 18h)
    const hours = publicacaoDate.getHours();
    ...
    if (isWeekend || !isBusinessHours) {
      toast({
        ...
        description: "A data/hora de publicação precisa ser em dia útil e horário comercial (08:00 - 12:00 / 14:00 - 18:00)",
      });
      return;
    }
    
    // Verificar se a solicitação está sendo feita com pelo menos 4 horas úteis de antecedência
    // Essa é uma simplificação para demonstração
    const diffHours = (publicacaoDate.getTime() - now.getTime()) / (1000 * 60 * 60);
    if (diffHours < 4) {
      toast({
        variant: "destructive",
        title: "Prazo insuficiente",
        description: "A solicitação deve ser feita com pelo menos 4 horas úteis de antecedência",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Sua solicitação de divulgação foi enviada com sucesso",
      });
    ...
    >
      <div className="max-w-3xl mx-auto">
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow-sm">
          ...
          <div>
            <Label htmlFor="dataPublicacao">Data/Hora de Publicação <span className="text-red-500">*</span></Label>
            <Input
              id="dataPublicacao"
              type="datetime-local"
            ...
            <p className="text-xs text-gray-500 mt-1">
              A data/hora de publicação deve ser em dia útil e horário comercial (08:00 - 12:00 / 14:00 - 18:00) 
              e com pelo menos 4 horas úteis de antecedência.
            </p>
          </div>

          <div>
            <Label htmlFor="mensagem">
              Mensagem a ser comunicada <span className="text-red-500">*</span>
            </Label>
            <Textarea
              id="mensagem"
              value={mensagem}
              onChange={(e) => setMensagem(e.target.value)}
              placeholder="Descreva a mensagem a ser comunicada, incluindo nome da ação, data, local, horário, pessoas envolvidas, objetivo e outras informações relevantes."
              required
...
export default SolicitarDivulgacao;
